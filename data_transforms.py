...
class Resize_3(object):
        def __init__(self, size):
            self.size = size
        def __call__(self, image, edge, gray):
            
            return image.resize((self.size, self.size)), \
                    edge.resize((self.size, self.size)), \
                    gray.resize((self.size, self.size)),
...
#         results = [image.crop((x1, y1, x1 + tw, y1 + th))]
#         if edge is not None:
#             results.append(edge.crop((x1, y1, x1 + tw, y1 + th)))
#             results.append(gray.crop((x1, y1, x1 + tw, y1 + th)))
#         return results
...
class ToTensor_3(object):
    ...
    def __call__(self, pic, pic2):
        ...
        # edge
        # pic2 = ImageOps.grayscale(pic2)
        edge = torch.ByteTensor(torch.ByteStorage.from_buffer(pic2.tobytes()))
        if pic2.mode == 'YCbCr':
            nchannel=3
        else:
            nchannel = len(pic2.mode)

        edge = edge.view(pic2.size[1], pic2.size[0], nchannel)
        edge = edge.transpose(0, 1).transpose(0, 2).contiguous()
        edge = edge.float().div(255)
        
        # gray
        pic3 = ImageOps.grayscale(pic)
        gray = torch.ByteTensor(torch.ByteStorage.from_buffer(pic3.tobytes()))
        
        gray = gray.view(pic3.size[1], pic3.size[0], 1)
        gray = gray.transpose(0, 1).transpose(0, 2).contiguous()
        gray = gray.float().div(255)
